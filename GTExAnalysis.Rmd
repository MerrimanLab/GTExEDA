...
tissueColours <- rainbow(40)[as.integer(as.factor(dict[tissueCols, SMTS]))]
head(tissueColours)
...
I am not convinced that this heatmap is yielding much of interest. As a final effort on this full dataset, let's consider using factor analysis. For the factor analysis, we will analyse the expression profiles of genes. Ideally, we should see similar expression patterns loading onto the same factors.

```{r}
fData <- logTransform(myGTEx, metaCols = c("Name", "Description", "GeneGroup"))
head(fData[, 1:10])

results <- factanal(fData, factors = 7)
results
```

An effort to visualise these using parallel coordinates:

```{r}
fLoadings <- fData %*% results$loadings

dict[, SMTSColours := rainbow(40)[as.integer(.SD[, SMTS])]]
fColours <- dict[rownames(fLoadings), SMTSColours]
MASS::parcoord(fLoadings, col = scales::alpha(fColours, 0.1))
```

Just for interest, let's compare the factor loadings with unrotated principal components:

```{r}
pca <- prcomp(fData)

par(mfrow = c(2, 1))
MASS::parcoord(fLoadings, col = scales::alpha(fColours, 0.05), main = "Factor Loadings")
MASS::parcoord(pca$x[, 1:10], col = scales::alpha(fColours, 0.05), main = "PCA Loadings")
...
These are definitely nice and colourful plots. But, I think we are being overwhelmed by the imbalanced tissue sampling. Let's pay more attention to the gene loadings:

```{r}
plotGeneLoadings <- data.frame(Z = rep(1:7, each = 40), 
                               loading = results$loadings[1:280], 
...
A tentative interpretation of these loadings might be:  

  - Z1: strong signal from the cancer genes (BRCA1 group).  
  - Z2: potentially a contrast between the TPPP and ABCG2 groups with the BARD1 gene.  
  - z3: Strong signal from TPPP2, and less so from it's group-mate TERT.  
  - z4: Strong signal from SMAD5 and ARHGEF12. No clear contrast of grouping of gene families.  
  - z5: Claer signal from the HMGA2 group of genes, contrasting with the TPPP group (PICK1, TPPP3, SCL6A3 genes).   
  - z6: Some signal from the ABCG2 group of genes.  
  - z7: The height family of genes are represented, somewhat contrasted with TPPP group.  
  
These are tentative interpretations at best. Mapping these interpretations back to the parallel coordinate plots is not obvious.

Parallel coords by tissue type
```{r}
byTissue <- as.data.table(fLoadings)
byTissue[, Tissue := dict[rownames(fLoadings), SMTS]]
head(byTissue)

par(mfrow = c(7, 4))
for (i in 1:25) {
    tx <- unique(byTissue[, Tissue])[i]
    print(sprintf("%s:  %s", as.character(tx), nrow(byTissue[Tissue == tx])))
    if (nrow(byTissue[Tissue == tx]) > 0) {
        MASS::parcoord(byTissue[Tissue == tx, 1:7, with = F], main = tx,
                       col = scales::alpha("steelblue", 0.2))
    }
}
par(mfrow = c(1, 1))
```

I just don't think we are seeing anything interesting here :( more thought is required - definitely. What this is saying, is that I cannot start making interesting comparisons between genes based solely on the boxplots that we see on the GTEx portal.
